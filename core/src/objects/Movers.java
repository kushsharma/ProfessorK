...
public class Movers {
	...
	public static int HOR = 0; // default
	public static int VER = 1;
	
	//current type
	public int MOVER_TYPE = HOR;	
	
	public Movers(World w, Vector2 pos, float size, int t){
		MOVER_TYPE = t;
		world = w;
		distance = size/2;
		position = pos;
	...
	private void init(World w){
		...
		if(MOVER_TYPE == HOR)
			texRegion = GameScreen.getInstance().getAssetLord().manager.get(AssetLord.game_atlas, TextureAtlas.class).findRegion("mover"); //new TextureRegion(tex);
			//texRegion = atlas.findRegion("level1-power");
		else if(MOVER_TYPE == VER)
			texRegion = GameScreen.getInstance().getAssetLord().manager.get(AssetLord.game_atlas, TextureAtlas.class).findRegion("mover");
	...
	public void update(float delta, float viewportWidth){
		...
		if(MOVER_TYPE == HOR){
			if((fixed.x - width/2 + distance) - body.getPosition().x < 0.00002f)
				body.setLinearVelocity(-Speed, body.getLinearVelocity().y);
			else if(body.getPosition().x - (fixed.x + width/2 - distance) < 0.00002f)
				body.setLinearVelocity(Speed, body.getLinearVelocity().y);
