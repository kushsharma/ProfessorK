...
//fix this shit
	...
	public CBackground(OrthographicCamera cam){
...
/*
 class ThingToRender {
        float x, y, z;
        public void draw(Batch batch, float sX, float sY) { drawIt(x + z * sX, y + z * sY); }
        public void drawIt(Batch batch, float x, float y) { dowhateveryouneedtodrawthethingatthislocation(); e.g.batch.draw(region, x, y); }
}
 
class Parallax {
        Array<ThingToRender> thingsToRender = new Array();
        void sort() { sortTheArrayBaseOnZBack(0)toFront(1); }
        void draw(Batch batch, float sX, float sY) { for (ThingToRender r : thingsToRender) r.draw(batch, sX, sY); }
}
*/
