...
public class Enemy {
	...
	private float height = 0.8f, width = 0.8f;
	...
	public Enemy(World w, Shape s, Vector2 p){
		_enemy = this;
		world = w;		
		shape = s;
		position = p;
	...
	public void init(){
		...
		playerSprite = new Sprite(atlas.findRegion("enemy-move-1"));
		getSkinTexture();		
	...
	private void getSkinTexture(){
		...
		TextureRegion playerMoveSheet[] = new TextureRegion[6];	
		
		playerMoveSheet[0] = atlas.findRegion("enemy-move-1"); //new TextureRegion(new Texture("level/enemy/enemy-move-1.png"));
		playerMoveSheet[1] = atlas.findRegion("enemy-move-2"); //new TextureRegion(new Texture("level/enemy/enemy-move-2.png"));
		playerMoveSheet[2] = atlas.findRegion("enemy-move-3"); //new TextureRegion(new Texture("level/enemy/enemy-move-3.png"));
		playerMoveSheet[3] = atlas.findRegion("enemy-move-4"); //new TextureRegion(new Texture("level/enemy/enemy-move-4.png"));
...
//		jumpAnime.setPlayMode(PlayMode.LOOP_PINGPONG);
	...
	public void hitBullet(int damage) {
		...
		if(LEFT_DIRECTION)
			body.applyLinearImpulse(0.7f, 0, body.getWorldCenter().x, body.getWorldCenter().y, true);
		else
			body.applyLinearImpulse(-0.7f, 0, body.getWorldCenter().x, body.getWorldCenter().y, true);
